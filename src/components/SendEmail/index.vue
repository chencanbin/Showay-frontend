<template>
  <div>
    <el-dialog
      ...
      append-to-body>
      <basic-container>
        <el-card class="box-card" style="width: 100%; margin-left: auto; margin-right: auto">
          <el-form label-width="80px" style="padding: 10px;">
            ...
            <el-form-item :label="$t('sendEmail.attachment')">
              <el-upload
                ...
                :auto-upload="false"
                action="">
                <el-button slot="trigger" size="small" type="primary">{{ $t('sendEmail.select_attachment') }}</el-button>
              </el-upload>
            </el-form-item>
            <el-form-item :label="$t('sendEmail.content')">
              <tinymce v-if="showTinymce" v-model="body" :height="300"/>
            </el-form-item>
          </el-form>
        </el-card>
        <div class="clearfix" style="text-align: center;margin-bottom: 10px">
          <el-button :loading="sendEmailLoading" type="primary" @click="sendEmail()">{{ $t('sendEmail.button') }}</el-button>
        </div>
      </basic-container>
    </el-dialog>
    <el-dialog
      ...
      <el-form label-width="80px">
        <el-form-item :label="$t('sendEmail.template')" prop="templateId">
          <el-select
            v-model="templateId"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="handleTemplateClose">{{ $t('common.cancelButton') }}</el-button>
        <el-button type="primary" @click="afterSelectTempate">{{ $t('common.confirmButton') }}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
