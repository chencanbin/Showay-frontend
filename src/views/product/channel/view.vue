...
export default {
  ...
  methods: {
    ...
    getViewData(params) {
      ...
      this.loading = true
      this.$api.channel.previewChannelCommission(this.id, this.params).then(res => {
    ...
    // },
    exportPDF() {
      const url = process.env.BASE_API + `/channelCommissionTable/${this.id}/export?&${param(this.params)}`
