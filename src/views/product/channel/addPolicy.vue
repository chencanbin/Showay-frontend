...
export default {
  ...
  methods: {
    ...
    validateNumber(rule, value, callback) {
      const reg = /^[0-9]+.?[0-9]*$/
      if (value > 100) {
        callback(new Error(this.$t('product.channel.set.add_policy.verify_message.commission_rate_exceed')))
      }
      if (reg.test(value)) {
        callback()
      }
      callback(new Error(this.$t('product.channel.set.add_policy.verify_message.commission_rate_is_number')))
    }
  }
}
</script>
