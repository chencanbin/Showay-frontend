<template>
  <span>
    ...
    <el-dialog
      ...
      :title="$t('client.insurance_policy.set.channel_expense')"
      width="400px"
      class="channel_expenses"
      append-to-body>
      <el-form>
        <el-card>
          <div slot="header" class="clearfix">
            <span style="margin-right: 10px; vertical-align: middle">{{ $t('client.insurance_policy.set.expense_item') }}</span>
          ...
          <el-form-item v-for="(item, index) in expenses" :key="index" style="margin-top: 10px;" class="input_item">
            ...
            <el-button :loading="removeExpensesLoading === item.id" icon="el-icon-remove-outline" size="small" style="min-width: 50px; font-size: 20px; padding: 6px; vertical-align: bottom;" @click="removeItem(item.id)"/>
          </el-form-item>
        </el-card>
      </el-form>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    getExpensesList() {
      this.$api.channel.fetchChannelExpenses({ insurancePolicy: this.policy.id }).then(res => {
        this.expenses = res.data.list
      })
    },
    addItem() {
      this.addExpensesLoading = true
      this.$api.channel.addChannelExpenses({ insurancePolicy: this.policy.id, type: 0, amount: 0 }).then(res => {
    ...
    removeItem(id) {
      ...
      this.$api.channel.deleteChannelExpenses(id).then(res => {
