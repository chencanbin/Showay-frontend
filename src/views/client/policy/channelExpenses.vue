...
<template>
  <span>
    ...
    <el-dialog v-el-drag-dialog :close-on-click-modal="false" :visible="dialogVisible" :before-close="handleClose" :title="$t('client.insurance_policy.set.channel_expense')" width="3.125rem" class="channel_expenses" append-to-body>
      <el-form>
        <el-card>
          <div class="header">
          ...
          <el-form-item v-for="(item, index) in expenses" :key="index" style="margin-top: 10px" class="input_item">
            ...
            <currency-input v-model="item.amount" symbol="HK$ " @input="expensesAmountChange(item)" class="input_inner_item" />
            <el-button :loading="removeExpensesLoading === item.id" size="small" icon="el-icon-remove-outline" class="action-button" @click="removeItem(item.id)" />
          </el-form-item>
        </el-card>
      </el-form>
    </el-dialog>
