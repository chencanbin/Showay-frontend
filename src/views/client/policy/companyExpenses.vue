...
export default {
  ...
  methods: {
    ...
    getExpensesList() {
      this.$api.commission.fetchExpenses({ insurancePolicy: this.policy.id }).then(res => {
        this.expenses = res.data.list
      })
    },
    addItem() {
      this.addExpensesLoading = true
      this.$api.commission.addExpenses({ insurancePolicy: this.policy.id, type: 0, amount: 0 }).then(res => {
    ...
    removeItem(id) {
      ...
      this.$api.commission.deleteExpenses(id).then(res => {
