        ...
        <el-tab-pane v-for="status in commissionPaymentStatus" :label="statusFormatter(status.id)" :name="status.id" :key="status.id">
          ...
          <el-table v-loading="loading" :height="height" :data="channelCommissionPayment.list" stripe border>
              ...
              <template slot-scope="scope">
                <el-form label-position="right" inline class="channelCommissionPayment-table-expand" label-width="80px">
                  ...
                  <el-form-item label="受保人:">
                    <span>{{ scope.row.insurancePolicy.beneficiary.name }}</span>
                  </el-form-item>
                  <el-form-item label="保额:">
                    <span style="margin-right: 5px">{{ getSymbol(scope.row.currency) }}</span><span>{{ formatterCurrency(scope.row.insurancePolicy.amountInsured) }}</span>
                  </el-form-item>
                  <el-form-item label="AFYP:">
                    <span style="margin-right: 5px">{{ getSymbol(scope.row.currency) }}</span><span>{{ formatterCurrency(scope.row.afyp) }}</span>
                  </el-form-item>
                  <el-form-item label="佣金率:">
                    <span>{{ formatterPercent(scope.row.commissionRate) }}</span>
                  </el-form-item>
                </el-form>
              </template>
            </el-table-column>
            ...
            <el-table-column label="上级渠道" prop="amount">
              <template slot-scope="scope">
                <div v-if="!scope.row.forSuperior"><span>{{ scope.row.superior.name }}</span></div>
              </template>
            </el-table-column>
            ...
            <el-table-column label="状态" min-width="100">
              <template slot-scope="scope">
                ...
                <el-tag v-if="activeName === '-1'">{{ statusFormatter(activeName) }}</el-tag>
...
export default {
  ...
  computed: {
    ...mapGetters(['loading']),
    ...mapState({ channelCommissionPayment: state => state.client.channelCommissionPayment })
  },
  created() {
  ...
  methods: {
    search: _.debounce(function() {
      this.getChannelCommissionPayment({ status: this.activeName, wildcard: this.wildcard })
    }, 500),
