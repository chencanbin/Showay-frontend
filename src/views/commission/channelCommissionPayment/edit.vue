    ...
    >
      <el-form 
        ...
        <el-form-item 
          ...
          prop="name">
          {{
            getSymbol(commissionPayment.currency) +
              formatterCurrency(commissionPayment.calculatedAmount)
          }}
        </el-form-item>
        <el-form-item 
          label="期序" 
  ...
  methods: {
    ...
    handleSubmit() {
      this.$refs["payment"].validate((valid) => {
        if (valid) {
          this.$api.channel
            .editChannelCommissionPayment(this.payment.id, this.payment)
            .then((_) => {
              this.$message({
                message: "操作成功",
                type: "success",
