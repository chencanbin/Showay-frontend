  ...
  <span id="clear">
    ...
    <el-dialog
      ...
      <el-form>
        ...
        <div style="font-size: 16px; font-weight: bold; color: rgb(144,0,0); text-align: left">
          <el-checkbox v-model="confirm" style="vertical-align: text-bottom; margin-right: 5px"/>
          * {{ $t('commission.payment.tooltip.clear') }}
        </div>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="handleCloseConfirmDialog">{{ $t('common.cancelButton') }}</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      imgSrc: '',
      rule: {
        chequeNumber: [{ required: true, message: this.$t('commission.payment.set.chequeNumber'), trigger: 'blur' }]
      },
      dialogVisible: false,
