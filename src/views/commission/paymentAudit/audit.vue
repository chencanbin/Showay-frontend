<template>
  <span>
    ...
    <el-dialog v-el-drag-dialog :close-on-click-modal="false" :visible="dialogVisible" :before-close="handleClose" center title="到账审核" width="400px">
...
export default {
  ...
  methods: {
    ...
    handleReject() {
      this.$api.commission
        .CommissionCreditReject(this.commissionCredit.id)
        .then((_) => {
          this.$message({
    ...
    handleClear() {
      this.$api.commission
        .CommissionCreditClear(this.commissionCredit.id)
        .then((_) => {
          this.$message({
            message: this.$t("common.success"),
            type: "success",
