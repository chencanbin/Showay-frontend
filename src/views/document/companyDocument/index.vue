      ...
      <div class="internal-file-list">
        <div class="file-menu-wrapper">
          <el-tree ref="tree" :load="loadFolder" :data="folderTree" :key="randomKey" :props="props" :default-expanded-keys="expandArray" node-key="id" highlight-current lazy style="margin-top: 10px" @node-click="nodeExpand">
            <span slot-scope="{ data }" :title="data.name" class="tree-item-wrapper">
...
export default {
  ...
  data() {
    return {
      tableHeight: this.hasPermission(100073)
  ...
  created() {
    this.getFolder(1);
    this.levelList = [
      { id: 1, data: { name: this.$t("document.company_file") } },
    ];
  },
  ...
  methods: {
    ...
    loadFolder(node, resolve) {
      ...
      if (node.level === 0) {
        this.folderTree = [
          {
            name: this.$t("document.company_file"),
            id: 1,
